...
This repository implements a **real-world OpenEnv environment** designed for training and evaluating AI agents on practical tasks. The environment follows the OpenEnv specification and exposes a standard interface using:

* `reset()`
* `step(action)`'
* `state()`

The goal is to simulate realistic workflows that humans perform and enable agents to learn through interaction, feedback, and rewards.
...
* `info`: Additional metadata (e.g., errors, logs)
...
* Objective: Basic task with minimal reasoning
* Expected Behavior: Single-step or simple logic
* Reward: Based on correctness

### 2. Medium Task

* Objective: Multi-step reasoning or decision-making
* Expected Behavior: Partial progress tracking
* Reward: Incremental rewards for intermediate steps

### 3. Hard Task

* Objective: Complex, real-world workflow
* Expected Behavior: Multi-step planning and execution
* Reward: Dense reward signals + final success score
...
* Negative reward for incorrect or inefficient actions
* Final reward for successful completion

This ensures agents learn from the full trajectory, not just the final outcome.

---

## 📊 Grading System

Each task includes a **deterministic grader** that evaluates performance on a scale of **0.0 to 1.0**.
...
* Reproducible
* Objective
* Aligned with real-world success criteria
...
1. Always call `reset()` before starting a task
2. Use `step(action)` iteratively until `done == True`
3. Use observations to decide next actions
4. Avoid invalid or undefined actions
5. Optimize for cumulative reward

---

## 🖥️ Inference Script Requirements

The `inference.py` script must:
...
* Read environment variables:
  ...
  * `MODEL_NAME`
  * `HF_TOKEN`
* Interact step-by-step with the environment
* Output logs in the required format:
...
* Working `Dockerfile`
* Environment starts successfully with `docker build` and `docker run`
* `/reset` endpoint responds with HTTP 200
...
* This is **not a game environment**
* Tasks must reflect real-world scenarios
* Rewards must not be sparse-only
* Graders must be deterministic and reproducible
* All components must pass `openenv validate`

---

## 🚀 Goal

The objective of this project is to build a **robust, realistic environment** that can:

* Train AI agents
* Evaluate model capabilities
* Simulate real-world decision-making workflows
...
For questions or issues, refer to the project README or contact the repository maintainer.
